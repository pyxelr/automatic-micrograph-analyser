...
// ACCEPT EXPANDABLE ARRAYS (FOR "DIR4" ARRAY)
setOption("ExpandableArrays", true);

// DEFINE VARIABLES
number_of_opened_image = 0;
analysis_options = "";
...
// ==========GET INPUT DIRECTORY AND ASSIGN THE FILES TO AN ARRAY==========
...
for (i=0; i<list.length; i++) {
    if  ((indexOf(toLowerCase(list[i]), ".tif")>0) || (indexOf(toLowerCase(list[i]), ".tiff")>0) || 
    	(indexOf(toLowerCase(list[i]), ".png")>0) || (indexOf(toLowerCase(list[i]), ".jpg")>0) || 
    	(indexOf(toLowerCase(list[i]), ".jpeg")>0) || (indexOf(toLowerCase(list[i]), ".bmp")>0)) {
...
// DEFINE VARIABLE FOR THE NUMBER OF OVERALL IMAGES IN THE DIRECTORY
number_of_images = nImages;

// IF THE FOLDER HAS NO IMAGES ASK TO RUN THE SCRIPT AGAIN
if (number_of_images<1)
{
	Dialog.create("Error!")
	Dialog.addMessage("There are no images in this directory.\n");
...
// ==========MAIN LOOP TO OPEN THE SUPPORTED IMAGE FILES AND RUN THE OPERATIONS==========
...
for (i=0; i<list.length; i++) {
    if  ((indexOf(toLowerCase(list[i]), ".tif")>0) || (indexOf(toLowerCase(list[i]), ".tiff")>0) || 
		...
		if (number_of_opened_image == 0 ) {
			setBatchMode(false);
		}

	    open(dir1+list[i]);
	    number_of_opened_image = i+1;
	    //GET IMAGE TITLE AND FIX THE EXTENSION
	    list2[i] = getTitle();
		dotIndex = indexOf( list[i], "." );
		list[i] = substring( list[i], 0, dotIndex );
		rename(list[i]+".tif");
		list[i] = getTitle();
...
// ==========CREATE OUTPUT DIRECTORY FOR EACH FILE==========
	    ...
	    dir3=dir2+list[i]+"_RESULTS("+year+"-"+month+"-"+dayOfMonth+" "+hour+";"+minute+";"+second+")"+File.separator;
	    File.makeDirectory(dir3);
	    // ADD OUTPUT FOLDER NAME TO AN ARRAY FOR "ANALYSIS INFO" SAVING FOR MULTIPLE IMAGES
	    dir4[i] = dir3;

// ==========SCALE SETTING (ONLY FOR THE 1ST IMAGE)==========

		if (number_of_opened_image == 1) {
...
// ==========SAVE RESULTS==========	    
	    ...
	    // SAVE RESULTS
	    if(chosenOption[0]==1) {
	    	saveAs("Results", dir3+"RESULTS_"+list[i]+".csv");
	    }
	    run("Clear Results");
	    run("Close");
        ...
        Table.update;
	    if(chosenOption[1]==1) {
	    	saveAs("Results", dir3+"SUMMARY_"+list[i]+".csv");
		}
	    run("Clear Results");
	    run("Close");
	    ...
	    if(image_processed==true) {
	    	saveAs("tiff", dir3+"PROCESSED_"+list[i]);
		}
		else {
			rename("PROCESSED_"+list[i]);
		}
	    // SAVE OUTLINED IMAGE
	    selectWindow(outlined_image_title);
	    if(image_outlined==true) {
	    	saveAs("tiff", dir3+"OUTLINED_"+list[i]);
		}
		else {
			rename("OUTLINED_"+list[i]);
		}
	    close();
...
// ==========RUN AND SAVE CLUSTER INDICATION (OPTIONALLY)==========
	    ...
	    if(chosenOption[4]==1) {
		    ...
		    selectWindow("PROCESSED_"+list[i]);
		    if(image_clustered==true) {
		    	saveAs("png", dir_cluster+"CLUSTERED_"+list[i]);
			}
			else {
				rename("CLUSTERED_"+list[i]);
			}
		    clustered_image_title = getTitle();
		    if (isOpen("Log")) {
		    	selectWindow("Log");
		    	saveAs("Text", dir_cluster+"CLUSTERS_"+list[i]+".txt");
		    	print("\\Clear");
		    	run("Close");
		    }	    
		    run("Set Scale...", "distance="+length_of_line+" known="+known_distance+" pixel=1 unit="+unit_of_length+" global");
...
// ==========RUN AREA DISTRIBUTION (OPTIONALLY)==========
		...
		if(chosenOption[3]==1) {
			...
			if (image_montage==true) {
				...
				saveAs("Tiff", dir_plot+"Montage of "+list[i]+".tif");
				close();
			}
			...
			for (slice=1; slice<=n; slice++) {
			    ...
			    if(image_slices==true) {
			    	saveAs("PNG", dir_plot+"Slice #"+slice+" of "+list[i]+".png");
				}
				else {
					rename("Slice #"+slice+" of "+list[i]+".png");
				}
			    rename(stack);
			    run("Analyze Particles...", "  size="+min_size+"-"+max_size+" circularity="+min_circularity+"-"+max_circularity+" show=Nothing slice");
			    saveAs("Results", dir_plot+"Overall Analysis Results of slice #"+slice+" of "+list[i]+".csv");
			    ...
			    Table.update;
			    saveAs("Results", dir_plot+"Distribution Plot (bin) values of slice #"+slice+" of "+list[i]+".csv");
			    run("Clear Results");
			    run("Close");
			    // GENERATE HIGH RESOLUTION PLOT
			    Plot.makeHighResolution("Histograms for slice #"+slice+" of "+list[i]+"",4.0);
			    // SAVE THE PLOT
			    saveAs("PNG", dir_plot+"Distribution Plot of slice #"+slice+" of "+list[i]+".png");
...
// ==========RUN INTERPARTICLE DISTANCES (OPTIONALLY)==========
		...
		if(chosenOption[2]==1) {
			...
			if(image_masks==true) {
				saveAs("tiff", dir3+"MASKS_"+list[i]);
			}
			else {
					rename("MASKS_"+list[i]);
			}
			...
			if(image_interparticle==true) {
				saveAs("png", dir_interparticle+"INTERPARTICLE_ADJACENCIES_"+list[i]);
			}
			else {
				rename("INTERPARTICLE_ADJACENCIES_"+list[i]);
			}
			//SAVE "LOG" WINDOW
			selectWindow("Log");
			...
			selectWindow("Results");
			saveAs("Results", dir_interparticle+"INTERPARTICLE_DISTANCE_MATRIX_"+list[i]+".csv");
...
// SAVE ANALYSIS SPECIFICATIONS (OPTIONALLY)
	if(chosenOption[6]==1) {		
		...
		print("Input folder: "+dir1+"");
		print("List of files in the input folder:");
		Array.print(list);   	 
		...
		print("Time taken for automatic processes: "+((getTime()-start2)/1000)*number_of_images+" seconds");
		saveAs("Text", dir3+"ANALYSIS_INFO("+year+"-"+month+"-"+dayOfMonth+" "+hour+";"+minute+";"+second+").txt");
		run("Close");
	}
	// DISPLAY SUCCESS WINDOW
	Dialog.create("Success!")
	Dialog.addMessage("Success!\nIt took "+(getTime()-start)/1000+" seconds to run all the actions and "+((getTime()-start2)/1000)*number_of_images+" seconds to run automatic scripts on "+number_of_images+" image.");
...
// SAVE ANALYSIS SPECIFICATIONS (OPTIONALLY)
	if(chosenOption[6]==1) {	
		for (i=0; i<dir4.length; i++) {	
			...
			print(""+TimeString+"");
			print("\n=== INPUT/OUTPUT FILES ===");
			print("Output subfolders in: "+dir2+"");
			print("Input folder: "+dir1+"");
			print("List of files in the input folder:");
			Array.print(list);
			print("Number of images: "+number_of_images+"");
			...
			print("Overall time taken for analysis: "+(getTime()-start)/1000+" seconds");
			print("Time taken for automatic processes: "+((getTime()-start2)/1000)*number_of_images+" seconds");
			saveAs("Text", dir4[i]+"ANALYSIS_INFO("+year+"-"+month+"-"+dayOfMonth+" "+hour+";"+minute+";"+second+").txt");
			run("Close");
	}

	// DISPLAY SUCCESS WINDOW
	Dialog.create("Success!")
	Dialog.addMessage("Success!\nIt took "+(getTime()-start)/1000+" seconds to run all the actions and "+((getTime()-start2)/1000)*number_of_images+" seconds to run automatic scripts on "+number_of_images+" images.");
	Dialog.addMessage("\nThe results were saved in subdirectories of:\n");
	Dialog.addMessage("      "+dir2); 
